...
class HomeController extends Controller
{
    ...
    public function verify_number(Request $request){
        ...
        $is_exist = User::where('phone',$phone)->count();
        if($is_exist==0){
    ...
    public function sendCode(Request $request){
        $phnOtp = mt_rand(100000, 999999);
        Session::put('phnOtp', $phnOtp);

        $msg = $phnOtp." is your ".env('APP_NAME')." Verification Code ";
        sendSMS($request->mobile, $msg);

        return response()->json(['status'=>'success', 'message' => 'Verification Code Sent Successfully.']);
    }
    
    public function verifyCode(Request $request){
        $sessphnOtp = Session::get('phnOtp');

        if($sessphnOtp == $request->code){
