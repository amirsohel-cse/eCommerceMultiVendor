...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if($request->session()->get('order_id') != null){
            ...
            elseif($request->payment_option == 'byanpay'){
                return redirect('/byanpay/payment');
            }
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        $tax = $subtotal*(5/100);
        $data['vat'] = $tax;
        $data['shipping_charge_vat_percentage'] = $shipping_charge_vat_percentage;
        $total = $subtotal + $tax;
        if($total< 50){
            $total+= $shipping_charge + $shipping_charge_vat;
            $data['shipping_cost'] = $shipping_charge;
            $data['shipping_charge_vat'] = $shipping_charge_vat;
        }else{
            $total = $total;
            $data['shipping_cost'] = 0;
            $data['shipping_charge_vat'] = 0;
        }
