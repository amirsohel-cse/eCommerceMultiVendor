...
{{--                        </li>--}}
                            ...
                            <div class="search-box flex-grow-1 px-4">
                                <form action="{{ route('search') }}" method="GET">
                                    <div class="d-flex position-relative">
                                        ...
                                        <div class="w-100">
                                            <input type="text" aria-label="Search" id="search" name="q" class="w-100" placeholder="Search" autocomplete="off" style="text-align: left">
                                        </div>
    ...
    <div class="nav-section header-sticky d-sm-none d-md-block d-none">
            ...
            <div class="container">
                <ul class="nav primary clone-main-menu" id="mercado_main" data-menuname="Main menu">
                    ...
                    @foreach (\App\Category::where('featured', 1)->get() as $key => $category)
                        ...
                        <a href="{{ route('products.category', $category->slug) }}" title="{{ __($category->name) }}" class="link-term mercado-item-title">{{ __($category->name) }}</a>
                        @if(count($category->subcategories)>0)
                        <div class="wrap-megamenu">
                            <div class="megamenu-content">
                                <div class="row">
                                    ...
                                    @endphp
                                    @foreach ($category->subcategories as $subcategory)
                                        <div class="custom-col-5 {{$i%2==0?'bgWhite':'bgGrey'}}">
                                            <div class="wrap-vertical-nav">
                                                <a href="{{ route('products.subcategory', $subcategory->slug) }}"><h2 class="menu-title">{{ __($subcategory->name) }}</h2></a>
                                                <ul>
                                                    @foreach ($subcategory->subsubcategories as $subsubcategory)
                                                        @php
                                                            foreach (json_decode($subsubcategory->brands) as $brand) {
                                                        ...
                                                        @endphp
                                                        <li class="menu-item"><a href="{{ route('products.subsubcategory',$subsubcategory->slug) }}">{{ __($subsubcategory->name) }}</a></li>
                                                    @endforeach
                                                </ul>
                                            </div>
                                        </div>
