...
@section('script')
    <script type="text/javascript">
          ...
          $.post("{{route('driver.details')}}",{ _token: '{{csrf_token()}}', driver_id :driverId})
              .done(function(data){
                  ...
                  $('#driver_details').html(
                      ...
                      "<p> Car plate No: <b>"+data.plate_no+"</b></p>"+
                      "<p> City: <b>"+data.city+"</b></p>"+
                      "<p> Area: <b>"+data.area+"</b></p>"+
      ...
      {{--$('#store_driver').on('click',function () {--}}
      {{--    var orderId = $('#order_id').val();--}}
      {{--    var driverId = $('#driver_id').val();--}}
      {{--    $('#message').html("");--}}
      {{--    document.getElementById('message').style="display:none";--}}
      {{--    $.post("{{route('driver.insert')}}",{ _token: '{{csrf_token()}}', order_id:orderId, driver_id :driverId})--}}
      {{--        .done(function(data){--}}
      {{--            console.log(data);--}}
      {{--            if(data.status==1){--}}
      {{--                $('#message').html("<p class='helper' style=\"color: green; text:bold;\"> "+"Driver Added successfully and email sent to the driver"+" </p>")--}}
      {{--                document.getElementById('message').style="display:block";--}}
      {{--            }else{--}}
      {{--                $('#message').html("<p class='helper' style=\"color: red; text:bold;\"> "+"Something Wrong"+" </p>")--}}
      {{--                document.getElementById('message').style="display:block";--}}
      {{--            }--}}
      {{--        })--}}
      {{--        .fail(function(xhr, status, error) {--}}
